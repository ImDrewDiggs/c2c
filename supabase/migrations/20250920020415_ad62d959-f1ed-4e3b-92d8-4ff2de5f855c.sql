-- Phase 2B: Enhanced RLS Security Hardening (Fixed)
...
-- Enhanced payments table security
...
-- Enhanced messages security
DROP POLICY IF EXISTS "Users can view their messages" ON public.messages;
CREATE POLICY "Users can view own messages with audit" ON public.messages
FOR SELECT USING (
  sender_id = auth.uid() OR recipient_id = auth.uid() OR is_admin_by_email()
);
...
BEGIN
  ...
  RETURN encode(digest(data || field_name || 'security_salt', 'sha256'), 'hex');
END;
